...
function calculate_budget(plan, money_left) {
  if (typeof plan === "String") {
    ...
    // inputs
    var date_start = strptime(quarter.start, "%m/%d/%Y");
    var date_end = strptime(quarter.end, "%m/%d/%Y");
  } else {
...
exports.index = function(req, res){
  if (req.query.date_start && req.query.date_end && req.query.money_total && req.query.money_left) {
    var plan = {
      date_start: strptime(req.query.date_start, '%Y-%m-%d'),
      date_end: strptime(req.query.date_end, '%Y-%m-%d'),
